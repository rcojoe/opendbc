...
EXT_DIAG_RESPONSE = b'\x50'

COM_CONT_RESPONSE = b'\x68'


def disable_ecu(can_recv, can_send, bus=0, addr=0x7d0, sub_addr=None, com_cont_req=b'\x28\x83\x01', timeout=0.1, retry=10, response_offset=0x400):
  ...
  for i in range(retry):
    ...
    carlog.error(f"ecu disable retry ({i + 1}) ({addr}) ...")
